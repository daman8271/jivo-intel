...
- **Potential GMV loss** (Swiggy) — value of demand that can't be served from current stock. Swiggy latest snapshot (2026-06-26): **₹16,961,734** potential GMV loss across 719 SKU-rows, with **198 SKU-rows at DOH < 7 days** (median DOH 26).
...
- **Per-platform raw inventory tables** (each with platform-native columns):
  ...
  - `amazon_inventory` (12,899 rows): vendor-central view — `sellable_on_hand_inventory/units, unsellable_*, aged_90_days_*, net_received, open_purchase_order_quantity, receive_fill_pct, vendor_confirmation_pct, sourceable_product_oos_pct, unfilled_customer_ordered_units, overall_vendor_lead_time_days`. business = "JIVO MART PVT LTD".
